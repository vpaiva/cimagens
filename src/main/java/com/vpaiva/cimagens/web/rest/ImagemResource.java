...
private final Logger log = LoggerFactory.getLogger(ImagemResource.class);
	...
	private Function<? super Imagem, ? extends ImagemDTO> convertUsuarioDTO() {
		return imag -> new ImagemDTO(imag.getId(), imag.getTipoImagem(), imag.getStatus(), Base64.getEncoder().encodeToString(imag.getImagem().getData()));
	}
	...
	public ResponseEntity<RestResult<List<ImagemDTO>>> listByUsuario(@PathVariable("idUsuario") String idUsuario) {
		...
		List<ImagemDTO> imagens = imagemService.listByUsuario(idUsuario).stream().map(convertUsuarioDTO()).collect(Collectors.toList());
	...
	public ResponseEntity<RestResult<List<ImagemDTO>>> listAll() {
		...
		List<ImagemDTO> listAll = imagemService.listAll().stream().map(convertUsuarioDTO()).collect(Collectors.toList());
